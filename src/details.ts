...
function lifecycleBadge(state: string): string {
  ...
  const color = s.includes("obsolete")
    ...
    : s.includes("deprecated")
      ...
      : s.includes("draft")
        ...
        : s.includes("effective") || s.includes("active")
...
function renderRequirement(d: Req): string {
  const meta = [
    d.categories.length
      ? `<strong>Categories:</strong> ${d.categories.map(esc).join(", ")}`
      : "",
    d.implementation
      ? `<strong>Implementation:</strong> ${esc(d.implementation)}`
      : "",
    d.references.length
      ? `<strong>References:</strong> ${d.references.map(esc).join(", ")}`
      : "",
  ]
    .filter(Boolean)
    .join(" &nbsp;·&nbsp; ");
  ...
  const impls = section(
    ...
    table(
      ...
      d.implementations.map((i) => [
        esc(i.element_kind),
        `<code class="fqn-link" data-fqn="${esc(i.fqn)}" data-kind="${esc(i.element_kind)}">${esc(i.fqn)}</code>`,
      ]),
    ),
  );
  ...
  const svcs = section(
    ...
    table(
      ["ID", "Title", "Verification", "Tests"],
      d.svcs.map((s) => {
        const ts = s.test_summary;
        const testCell =
          ts.passed + ts.failed + ts.skipped + ts.missing === 0
            ? "—"
            : `${statusIcon("passed")} ${ts.passed} ${statusIcon("failed")} ${ts.failed} ${statusIcon("skipped")} ${ts.skipped}`;
        const svcLink = detailsLink(s.id, "svc");
        const stateBadge =
          s.lifecycle_state &&
          s.lifecycle_state.toLowerCase() !== "effective" &&
          s.lifecycle_state.toLowerCase() !== "active"
...
function renderSvc(d: Svc): string {
  ...
  const reqs = section(
    ...
    table(
      ["ID", "Title", "Lifecycle"],
      d.requirement_ids.map((r) => [
        detailsLink(r.id, "requirement"),
        esc(r.title),
...
export class DetailsViewProvider implements vscode.WebviewViewProvider {
  ...
  resolveWebviewView(
    ...
    webviewView.webview.onDidReceiveMessage(async (msg) => {
      if (msg.command === "search") {
        ...
        vscode.commands.executeCommand("reqstool.openDetails", {
          id: msg.id,
          type: msg.type,
        });
        ...
        const classIdx = isMethodOrField
          ? segments.length - 2
          : segments.length - 1;
        const searchSymbol =
          segments[classIdx] ?? segments[segments.length - 1];
        const memberName = isMethodOrField
  ...
  private _update(data: Record<string, unknown>): void {
    ...
    const html = DetailsViewProvider._renderDetail(
      data as unknown as DetailsData,
    );
  ...
  private static _renderDetail(data: DetailsData): string {
    if (data.type === "requirement") {
      return renderRequirement(data);
    }
    if (data.type === "svc") {
      return renderSvc(data);
    }
