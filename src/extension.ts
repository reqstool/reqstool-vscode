...
function logServerInfo(
  ...
  channel.appendLine(
    `[${ts}] reqstool ${version ?? "unknown"} (source: ${source}, path: ${executablePath})`,
  );
}
...
export async function activate(context: vscode.ExtensionContext) {
  ...
  const bundledVersion =
    (
      context.extension.packageJSON.reqstoolVersion as string | undefined
    )?.trim() || undefined;
  ...
  const isCommandConfigured = !!(
    cmdInsp?.globalValue ||
    cmdInsp?.workspaceValue ||
    cmdInsp?.workspaceFolderValue
  );
  ...
  if (!isCommandConfigured) {
    ...
    const neverChosen =
      !sourceInsp?.globalValue &&
      !sourceInsp?.workspaceValue &&
      !sourceInsp?.workspaceFolderValue;
    if (neverChosen) {
      const managedBinPath = getManagedBinPath(context);
      const managedBinVersion = fs.existsSync(managedBinPath)
        ? await getInstalledVersion(managedBinPath, timeout)
        : undefined;
      await showServerSourcePicker(
        systemVersion,
        managedBinVersion,
        bundledVersion,
      );
  ...
  if (
    ...
    bundledVersion &&
    systemVersion &&
    semverLt(systemVersion, bundledVersion)
  ) {
    vscode.window
      .showWarningMessage(
  ...
  const traceOutputChannel =
    vscode.window.createOutputChannel("reqstool Trace");

  logServerInfo(outputChannel, activeVersion, activeSource, executable);

  const langStatus = vscode.languages.createLanguageStatusItem(
    "reqstool.status",
    { language: "*" },
  );
  ...
  langStatus.command = {
    command: "reqstool.selectServerSource",
    title: "Select Server Source",
  };
  ...
  context.subscriptions.push(
    ...
    vscode.commands.registerCommand("reqstool.outline.scopeProject", () =>
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      ...
      async (
        args: { id?: string; ids?: string[]; type: string } | undefined,
      ) => {
        const id = args?.id ?? args?.ids?.[0];
        if (!id || !args?.type) {
          vscode.window.showInformationMessage(
            "reqstool: Open Details must be invoked from a code lens or hover link.",
          );
          return;
        }
        if (!client) {
        ...
        try {
          ...
          if (!data) {
            vscode.window.showWarningMessage(
              `reqstool: no details found for ${id}`,
            );
            return;
          }
          ...
          vscode.window.showErrorMessage(
            `reqstool: failed to load details for ${id}: ${err}`,
          );
        }
      },
    ),
  ...
  const serverVersion =
    client.initializeResult?.serverInfo?.version ?? activeVersion;

  if (serverVersion && serverVersion !== activeVersion) {
    langStatus.detail = `${serverVersion} (${activeSource})`;
    const ts = new Date().toISOString();
    outputChannel.appendLine(
      `[${ts}] reqstool server version: ${serverVersion}`,
    );
  }
...
async function loadStatusStats(
  ...
  try {
    ...
    const ts = new Date().toISOString();
    channel.appendLine(
      `[${ts}] URNs loaded: ${urns.map((u) => u.urn).join(", ")}`,
    );
  } catch {
  ...
  const items: vscode.QuickPickItem[] = [
    {
      ...
      detail:
        "Use system reqstool if installed; otherwise fall back to the version packaged with this extension.",
    },
    {
...
async function ensureManagedVenv(
  ...
  const storedReqstoolVersion =
    context.globalState.get<string>("envReqstoolVersion");
  const currentVersion = context.extension.packageJSON.version as string;
  const currentReqstoolVersion =
    (context.extension.packageJSON.reqstoolVersion as string | undefined) ?? "";
  if (
    (storedVersion !== currentVersion ||
      storedReqstoolVersion !== currentReqstoolVersion) &&
    ...
    try {
      await vscode.window.withProgress(
        ...
        async () => {
          ...
          const reqstoolVersion = context.extension.packageJSON
            .reqstoolVersion as string | undefined;
          const packageSpec = reqstoolVersion
            ? `reqstool==${reqstoolVersion}`
            : "reqstool";
...
async function findPython(): Promise<string | undefined> {
  ...
  for (const candidate of ["python3", "python"]) {
    const found = await new Promise<boolean>((resolve) =>
      execFile(candidate, ["--version"], { timeout: 3000 }, (err) =>
        resolve(!err),
      ),
    );
    if (found) {
...
const VERSION_RE =
  /(\d{1,10}(?:\.\d{1,10}){1,3}(?:(?:[abc]|rc)\d{0,10})?(?:\.(?:dev|post)\d{0,10})?(?:\+[\w.]+)?)/;

export function parseVersionFromVersionOutput(
  stdout: string,
): string | undefined {
